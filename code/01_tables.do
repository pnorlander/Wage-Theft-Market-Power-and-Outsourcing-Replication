...
* Output Table 2 - handle both subcontractor and subcontractor_2019 variable names
* The coefficient will appear as either 1.subcontractor or 1.subcontractor_2019 depending on source
esttab t2_1 t2_2 t2_3 t2_4 t2_5 t2_6 using "$tables/table2.tex", ///
    keep(2.hhicat 3.hhicat 2.round_wage_level 3.round_wage_level 4.round_wage_level logimm 1.subcontractor 1.subcontractor_2019) ///
    order(2.hhicat 3.hhicat 2.round_wage_level 3.round_wage_level 4.round_wage_level logimm 1.subcontractor 1.subcontractor_2019) ///
    coeflabels(2.hhicat "Moderate HHI" 3.hhicat "High HHI" ///
               ...
               1.subcontractor "Subcontractor" 1.subcontractor_2019 "Subcontractor") ///
...
esttab tabC* using "$tables/table4.tex", replace ///
    ...
    se star(* 0.1 ** 0.05 *** 0.01) ///
    stats(Year Occ CommuteZone N widstat jp, fmt(%9.0fc) label("Year FE" "Occupation FE" "Commuting Zone FE" "Observations" "Kleibergen-Paap F" "Hansen J p-value")) ///
...
estout tabC* using $tables/table_a1.tex, replace `dropvars' `rename' ///
  order(HHI Subcontractor SubcontractorXHHI Log(H-1Bs)) `addons' ///
  prefoot(\hline) stats(Year Occ CommuteZone Firm N, fmt(%9.2gc %9.0fc)) eform
...
estout tabE* using $tables/table_a2_panelA.tex, replace `rename' keep(`keeps') order(`keeps') `addons' ///
  prefoot(\hline) stats(Year Occ Firm r2 k_absorb N, fmt(%9.2gc %9.2f %9.0fc %9.0fc))
...
estout tabI* using $tables/table_a4.tex, replace `dropvars' `rename' ///
  order(HHI Subcontractor SubcontractorXHHI Log(H-1Bs)) `addons' ///
  prefoot(\hline) stats(Year Occ CommuteZone Firm r2 N, fmt(%9.2gc %9.2f %9.0fc))
...
estout tabM* using $tables/table_a7.tex, replace `rename' keep(`keeps') order(`keeps') `addons' ///
  prefoot(\hline) stats(Year Occupation CommutingZone r2 N, fmt(%9.2gc %9.2f %9.0fc))
...
estout tabN* using $tables/table_a8.tex, replace `rename' keep(`keeps') order(`keeps') `addons' ///
  prefoot(\hline) stats(Year Industry CommutingZone r2 N, fmt(%9.2gc %9.2f %9.0fc))
